  ...
  const [{ isDragging }, drag] = useDrag({
    item: { index, name, currentColumnName, type: "Our first type" },
      ...
      if (dropResult) {
        ...
        const { LIST, BOARD } = COLUMN_NAMES;
        switch (name) {
          case LIST:
            changeItemColumn(item, LIST);
            break;
          case BOARD:
            changeItemColumn(item, BOARD);
            break;
          default:
            break;
        }
  ...
  const [{ isOver, canDrop }, drop] = useDrop({
    ...
    canDrop: (item) => {
      ...
      return (
        ...
        (currentColumnName === LIST && title === BOARD) ||
        (currentColumnName === BOARD && title === LIST)
      );
    }
  });
